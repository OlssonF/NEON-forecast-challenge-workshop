...
## 1. Read in the data
We start forecasting by first looking at the historic data - called the 'targets'. This data is available near real-time, with the latency of approximately 24-48 hrs. We will start by reading in the data from the targets file. 
...
The columns of the targets file show the time step (daily for aquatics challenge), the 4 character site code (`site_id`), the variable being measured, and the mean daily observation. To look at just the lakes we can subset the targets and aquatic sites to those which have the `field_site_subtype` of `Lake`. 
...
For Random Walk (RW) forecasts (i.e.,, persistence), we simply set the forecast value be the value of the last observation. This is fine if we have data observed to yesterday but this often isn't the case. To make sure that the forecast uncertainty is reasonable, given the time since an observation, we start the forecast from the last observations, run forward until today (if necessary) days into the future, and for 30 days in the future.  In practice this means telling fable to forecasts 30 days + the number of days since last observation and cutting off all the day that are in the past before submitting the forecast.
...
```
## Submit forecast 
Files need to be in the correct format with metadata for submission. The forecast organizers have created tools to help aid in the submission process. These tools can be downloaded from Github using `remotes::install_github(eco4cast/neon4cast)`.
