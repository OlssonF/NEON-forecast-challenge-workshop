...
# 3. Our first forecast: Persistence
...
```{r, eval = F, comment='Note: Running the next chunk will return forecasts with all NAs. This is because there was no observation yesterday.'}
RW_model <- targets %>% 
...
Specifying the model and forecasts in this way would be fine if we have data observed to yesterday but this often isn't the case. Therefore, the forecast should instead be started at the last observation, to make sure that the forecast uncertainty is reasonable. The forecast is run from that observation through today for 30 days in the future.  In practice this means telling fable to forecasts 30 days + the number of days since last observation and cutting off all the day that are in the past before submitting the forecast.
