...
We start forecasting by first looking at the historic data - called the 'targets'. This data is available near real-time, with the latency of approximately 24-48 hrs. Here is how you read in the data from the targets file. 
...
* We could think about what the historic data tells us about this time of year. November this year is likely to be similar to November last year (Climatology/Seasonal Naive Model)
* And we could also look at the lake variables' relationship(s) with other variable. Could we use existing forecasts about the weather to generate forecasts about lake variables (Linear Model with Co-variates).
...
# Our first forecast: Persistence

This first forecast will use a method from the `fable` R package which is installed via `fpp3` package. The Fable package implements a range of different forecasting methods including Persistence Models, Moving Average Models, ARIMA Models and Time Series Models. The package integrates with `tidyverse` syntax and has good documentation and examples found [here](https://otexts.com/fpp3/).
...
library(neon4cast) # install via remotes::install_github('eco4cast/neon4cast)
...
Note: Running the next chunk will return forecasts with all NAs. This is because there was no observation yesterday.
```{r, eval = T}
RW_model <- targets %>% 
...
RW_daily_forecast <- RW_model %>% 
  forecast(h = 30)
head(RW_daily_forecast)
```

The forecast produces all NAs. Why would that be? If you look at the targets data we can see that yesterday did not have an observation, so the model cannot produce a persistence forecast. 
...
You can see that the site-variable combinations have different start dates, based on when the last observation was taken. We want to fit each site and variable model separately depending on its start date and calculated horizon. To do this I have written a custom function that runs the RW forecast. Within this function we:
...
* Forecast: Then using this model, we run a forecast! We can specify whether bootstrapping is used and the number of bootstraps (`bootstrap = T`). Bootstrapping is where we run the forecast multiple times with a random error term drawn from the residuals of the fitted model. Doing this repeatedly, we obtain many possible futures (an ensemble forecast). We decide how many of these possibilities to simulate with the `times=...` argument. The code to forecast using the specified model and bootstrapping is: 
`forecast <- RW_model %>% forecast(h = 30, bootstrap = T, times = 200)`
...
The forecast output from `fable` needs modifying slightly to fit the Challenge stanards. 
...
## TASKS
Possible modifications to the simple linear model (lines x-x):
...
# Another model: Seasonal naive model
...
## TASKS
Possible modifications to the Seasonal Naive Model (lines x-x):
