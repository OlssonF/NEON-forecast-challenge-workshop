...
Information on the NEON sites can be found in the `NEON_Field_Site_Metadata_20220412.csv` file on GitHub. It can be filtered to only include aquatic sites. This table has information about the field sites, including location, ecoregion, information vegetation type (`phenocam_vegetation`), canopy height, and soil type. 
...
* We could use information about recent conditions to predict the future. What is happening today is usually a good predictor of what will happen tomorrow (Model 2 - Persistence). 
* We could also look at the variable's relationship(s) with other variables. Could we use existing forecasts about the weather to generate forecasts about terrestrial fluxes (Model 1 - Linear Model with Co-variates).
* And we could think about what the historic data tells us about this time of year. Is this time of year likely to be similar the same period last year (Model 3 - Climatology/Seasonal Naive Model)
...
We will generate a `nee` forecast using `air_temperature` and `solar_radiation` as a co-variates. As an example of the workflow we will work with 1 site - ABBY (Abby Road Evergreen forecast).
...
noaa_past_mean_example <- noaa_past |> 
...
noaa_future_daily_example |> 
  ...
  geom_line(data = subset(noaa_past_mean_example, site_id == 'ABBY'), colour = 'darkblue') +
...
fit <- lm(site_target$nee ~ site_target$air_temperature + site_target$shortwave)
    
# use linear regression to forecast water temperature for each ensemble member
forecasted_nee <- fit$coefficients[1] + 
  (fit$coefficients[2] * noaa_future_site$air_temperature) + 
  (fit$coefficients[3] * noaa_future_site$shortwave)

```
We can loop through this for each site to create a site-wise forecast of NEE based on a linear model and each forecasted air temperature and shortwave. We can run this forecast for each site and then bind them together to submit as one forecast. 
...
for(i in 1:length(site_data$field_site_id)) {  
  ...
  example_site <- site_data$field_site_id[i]
  ...
  NEE <- tibble(datetime = noaa_future_site$datetime,
  ...
  message(example_site, ' NEE forecast run')
...
lm_forecast %>% 
  ...
  geom_point(data = subset(targets, site_id == 'ABBY'),
...
```{r, echo=F, warning=F}
targets_lm |> 
  filter(site_id %in% c('ABBY', 'BART')) |> 
  ggplot( aes(x=air_temperature, y= nee, colour = shortwave)) +
  facet_wrap(~site_id, scales = 'free') +
