...
We will generate a `nee` forecast using `air_temperature` and `solar_radiation` as a co-variates. As an example of the workflow we will work with 1 site - DELA (Dead Lake deciduous forest).
...
noaa_future_daily_example |> 
  ...
  geom_line(data = subset(noaa_past_mean_example, site_id == 'DELA'), colour = 'darkblue') +
...
Note: This loop takes a few minutes to execute so start it running as soon as possible...
```{r}
...
```{r, echo = F, warning = F}
lm_forecast %>% 
  filter(site_id %in% c('DELA', 'BARR')) |> 
  ggplot(aes(x=datetime, y=prediction, group = parameter)) + 
  geom_point(data = subset(targets, site_id %in% c('DELA', 'BARR')),
...
Is the linear model a reasonable relationship between air temperature and water temperature? Would some non-linear relationship be better? What about using maximum shortwave to predict NEE? Or including additional parameters? 
