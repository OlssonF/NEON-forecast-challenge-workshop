...
targets_ts <-  as_tsibble(targets, key = c('variable', 'site_id'), index = 'time') %>%
  ...
  group_by_key() %>% 
  # Create a copy of the column that can be filled in
  mutate(observed_na = observed) %>%
  # fills gaps with the last non-NA value
  tidyr::fill(observed_na, .direction = 'down') %>%
  # Retain all the NAs apart from today which needs to have a value (will be the previous non-NA value)
  mutate(observed = ifelse(time == Sys.Date(), observed_na, observed)) %>%
  select(-observed_na)
