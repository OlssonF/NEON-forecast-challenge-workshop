...
simple_SN
```
The output from this is a `fable` with the forecast as an S3 distribution, which gives the mean and variance of the prediction. 
...
This function extracts the variance from the ditribution and calculates the standard deviation (sigma). 
...
convert.to.efi_standard <- function(df){
  ...
  df %>% 
    ...
    mutate(family = "normal",
           start_time = start_time = min(time) - lubridate::days(1)) %>%
...
```{r, message = F, warning = F, echo = F, fig.cap="Figure: Example 'seasonal naive' forecasts for Lake Suggs (FL). Shade values show 95% confidence intervals"}
simple_SN_efi %>% 
  filter(site_id == 'SUGG') %>%
