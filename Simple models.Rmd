...
The first forecasts assume a normally distributed forecast or require a transformation.
An alternative is to use bootstrapping to obtain many possible outcomes. Here we run it 50 times across a 35 day horizon. 

```{r, eval = FALSE, error = TRUE}
targets_fit_boot <-  targets_ts %>%
  filter(variable == 'temperature', site_id == 'PRPO') %>%
  model(
    `Naïve` = NAIVE(observed)
  )

bootstapped <- targets_fit_boot %>% forecast(h = 35, times = 50, bootstrap = TRUE)


autoplot(bootstapped, subset(targets_ts, site_id == 'PRPO' & variable == 'temperature')) +
  coord_cartesian(xlim = c(ymd('2022-08-01'),
                           ymd('2022-09-10')))
```
